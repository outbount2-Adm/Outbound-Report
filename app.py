...
if 'processed_result' in st.session_state:
    ...
    with st.container():
        ...
        with f_col1: sel_plat = st.selectbox("Platform", ["Semua"] + sorted(res_df['Platform'].dropna().unique().tolist()))
        with f_col2: sel_brand = st.selectbox("Brand", ["Semua"] + sorted(res_df['Brand'].dropna().unique().tolist()))
        with f_col3:
            status_col = next((c for c in res_df.columns if 'status' in c.lower()), None)
            sel_status = st.selectbox("Status", ["Semua"] + sorted(res_df[status_col].dropna().unique().tolist())) if status_col else "Semua"
