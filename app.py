    ...
    if uploaded_files:
        ...
        try:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                # ========================================================
                # KODE PERBAIKAN: PERHITUNGAN METRICS (PENDING, KURIR, KATEGORI)
                # ========================================================
                raw_ho_df = dfs.get('ho_outbound', pd.DataFrame())
                master_df = dfs.get('master', pd.DataFrame())

                # 1. PENDING ORDER (Pencarian Kolom Fleksibel & Pembersihan Karakter String)
                val_pending = 0
                ...
                # 2. KURIR INSTAN
                val_kurir_instan = 0
                col_kurir = next((c for c in raw_ho_df.columns if 'kurir' in c.lower() or 'carrier' in c.lower() or 'logistics' in c.lower() or 'expedisi' in c.lower()), None)
                ...
                if col_kurir and col_qty:
                    ...
                    val_kurir_instan = int(pd.to_numeric(raw_ho_df.loc[is_instan, col_qty].astype(str).str.replace(r'[^\d.-]', '', regex=True), errors='coerce').fillna(0).sum())

                # 3. KATEGORI (JC ENABLER, FULFILMENT, OTHER)
                val_jc_enabler = 0
                val_jc_fulfilment = 0
                val_other = 0
                ...
                col_master_platform = next((c for c in master_df.columns if 'platform' in c.lower()), None) if not master_df.empty else None
                col_master_brand = next((c for c in master_df.columns if 'brand' in c.lower()), None) if not master_df.empty else None
                col_master_category = next((c for c in master_df.columns if 'category' in c.lower() or 'kategori' in c.lower()), None) if not master_df.empty else None
                ...
                if all([col_ho_platform, col_ho_brand, col_master_platform, col_master_brand, col_master_category, col_qty]):
                    ...
                    m_plat_clean = master_df[col_master_platform].astype(str).str.strip().str.lower()
                    m_brand_clean = master_df[col_master_brand].astype(str).str.strip().str.lower()
                    m_cat_clean = master_df[col_master_category].astype(str).str.strip().str.lower()
                    
                    # Buat mapping master lokal untuk perhitungan spesifik file HO
                    temp_master = pd.DataFrame({'plat': m_plat_clean, 'brand': m_brand_clean, 'cat': m_cat_clean})
                    master_mapping = temp_master.set_index(['plat', 'brand'])['cat'].to_dict()
                    ...
                    qty_clean = pd.to_numeric(raw_ho_df[col_qty].astype(str).str.replace(r'[^\d.-]', '', regex=True), errors='coerce').fillna(0)
                    
                    val_jc_enabler = int(qty_clean[raw_ho_df['Kategori_Final'] == 'jc enabler'].sum())
                    val_jc_fulfilment = int(qty_clean[raw_ho_df['Kategori_Final'] == 'jc fulfilment'].sum())
                    val_other = int(qty_clean[raw_ho_df['Kategori_Final'] == 'other'].sum())
                else:
                    # Fallback jika format master tidak cocok: Memakai master_category_map yang di-load di Tahap 1
                    def lookup_category_from_master(row):
                        ...
                        if pb2_val in master_category_map: return master_category_map[pb2_val]
                        elif (p_val, b2_val) in master_category_map: return master_category_map[(p_val, b2_val)]
                        return "Other"
