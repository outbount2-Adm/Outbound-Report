...
<style>
    ...
    .officer-name { color: #2563EB; font-weight: 700; }
...
with st.container():
    ...
    with col_h1:
        ...
        st.markdown(f'<div class="subtitle">Logistics Automation Dashboard • Selamat datang kembali!</div>', unsafe_allow_html=True)
    with col_h2:
        st.markdown(f"""
            <div style="text-align: right; padding-top: 0.5rem;">
                <span style="color: #64748B; font-size: 0.9rem;">Hari ini: <span class="officer-name">{current_date}</span></span>
            </div>
    ...
    with col_adm1:
        admin_input_temp = st.text_input("Officer Aktif", value=st.session_state['saved_admin'], placeholder="Ketik nama Officer / Admin...")
    with col_adm2:
        st.write("") 
...
with st.container():
    ...
    st.markdown('<div style="color: #475569; font-size: 14px; margin-bottom: 15px;">Seret dan lepas file sumber Anda di bawah ini.</div>', unsafe_allow_html=True)
        ...
        if uploaded_files:
            ...
            try:
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    ...
                    # --- URUTAN ASCENDING PADA AVG TIMES PROSES KURIR ---
                ...
                # Variabel Metrik untuk Tampilan Dashboard Profesional
...
# ==========================================
# 6. TAMPILAN DASHBOARD & PREVIEW PROFESIONAL
# ==========================================
if 'processed_result' in st.session_state:
    ...
    with st.container():
        st.markdown('<div class="modern-card">', unsafe_allow_html=True)
        st.markdown('<h3>📊 Ringkasan Eksekutif & Metrik Kinerja</h3>', unsafe_allow_html=True)
